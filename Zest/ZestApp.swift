...
struct ZestApp: App {
    ...
    @State private var mcpService = MCPService.shared
    ...
    private func initializeApp() async {
        ...
        if MCPSettingsManager.shared.enabled {
            mcpService.startHealthMonitor()
        }
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationShouldHandleReopen(_ sender: NSApplication, hasVisibleWindows flag: Bool) -> Bool {
        // When user clicks dock icon or menubar "Open Zest" and no visible windows
        if !flag {
            // Find and show the main window
            for window in sender.windows {
                if window.title == "Zest" {
    ...
    var body: some View {
        ...
        NavigationSplitView {
            VStack(spacing: 0) {
                ...
                // Control section at bottom - current mode badge + proxy control
                VStack(spacing: 0) {
                    Divider()
                    ...
                    // Proxy Control Button - only in local proxy mode
                    if modeManager.isLocalProxyMode {
                        ProxyControlButtonCompact(viewModel: viewModel)
    ...
    var body: some View {
        HStack {
            Circle()
                ...
                .glowPulse(color: statusColor, isActive: modeManager.connectionStatus == .connected)
...
struct ProxyStatusRow: View {
    ...
    private var statusColor: Color {
        viewModel.proxyManager.proxyStatus.running ? ZestColors.accentGreen : ZestColors.textTertiary
    }
        ...
        HStack {
            if viewModel.proxyManager.isStarting {
                ...
                Circle()
                    .fill(statusColor)
                    .frame(width: 8, height: 8)
                    .glowPulse(color: statusColor, isActive: viewModel.proxyManager.proxyStatus.running)
            }
