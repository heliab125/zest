...
//  Updated with Zest dark theme colors.
//
...
struct ZestPrimaryButtonStyle: ButtonStyle {
                    ...
                    .fill(
                        isEnabled
                            ? (configuration.isPressed
                                ? ZestColors.accentBlue.opacity(0.8)
                                : ZestColors.accentBlue)
                            : ZestColors.textTertiary
                    )
            )
            .scaleEffect(configuration.isPressed ? 0.97 : 1.0)
            .animation(ZestAnimations.micro, value: configuration.isPressed)
...
struct ZestSecondaryButtonStyle: ButtonStyle {
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .background(
                RoundedRectangle(cornerRadius: 8)
                    .fill(configuration.isPressed ? ZestColors.surfaceHover : ZestColors.surface)
            )
            .overlay(
                RoundedRectangle(cornerRadius: 8)
                    .stroke(ZestColors.surfaceBorder, lineWidth: 1)
            )
...
struct SubtleButtonStyle: ButtonStyle {
    var hoverColor: Color = ZestColors.surfaceHover
