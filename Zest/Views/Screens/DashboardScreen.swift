...
struct DashboardScreen: View {
    ...
    private var connectionStatusColor: Color {
        switch modeManager.connectionStatus {
        case .connected: return ZestColors.accentGreen
        case .connecting: return ZestColors.accentOrange
        case .disconnected: return ZestColors.textTertiary
        case .error: return ZestColors.accentRed
        }
    }
    ...
    private var quotaOnlyKPISection: some View {
        LazyVGrid(columns: [GridItem(.adaptive(minimum: 180), spacing: 16)], spacing: 16) {
            ...
            KPICard(
                ...
                icon: "chart.bar.fill",
                color: lowestQuotaPercentage > 50 ? ZestColors.accentGreen : (lowestQuotaPercentage > 20 ? ZestColors.accentOrange : ZestColors.accentRed),
                index: 2
            )
...
struct GettingStartedStepRow: View {
    ...
    var body: some View {
        HStack(spacing: 16) {
            ZStack {
                Circle()
                    .fill(step.isCompleted ? ZestColors.accentGreen : ZestColors.accentBlue.opacity(0.15))
                ...
                if step.isCompleted {
                    ...
                    Image(systemName: step.icon)
                        .font(.system(size: 16))
                        .foregroundStyle(ZestColors.accentBlue)
                }
            }
                ...
                HStack {
                    ...
                    if step.isCompleted {
                        Image(systemName: "checkmark.circle.fill")
                            .foregroundStyle(ZestColors.accentGreen)
                            .font(.caption)
                    }
                }
    ...
    var body: some View {
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 14)
                .stroke(
                    isHovered ? color.opacity(0.3) : ZestColors.surfaceBorder,
                    lineWidth: 0.5
                )
        )
...
struct QuotaProviderRow: View {
    ...
    private var quotaColor: Color {
        if lowestQuota > 50 { return ZestColors.accentGreen }
        if lowestQuota > 20 { return ZestColors.accentOrange }
        return ZestColors.accentRed
    }
        ...
        HStack(spacing: 12) {
            ...
            HStack(spacing: 6) {
                Circle()
                    .fill(quotaColor)
                    .frame(width: 8, height: 8)
                    .glowPulse(color: quotaColor, isActive: lowestQuota < 20)
