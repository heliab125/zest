        ...
        VStack(spacing: 16) {
            ...
            LazyVGrid(
                ...
                LinkCard(
                    ...
                    icon: "link",
                    color: .blue,
                    url: URL(string: "https://github.com/heliab125/zest")!
                )
    ...
    var body: some View {
        Section {
            HStack {
                VStack(alignment: .leading, spacing: 4) {
                    HStack(spacing: 6) {
                        Text("mcp.settings.title".localized())

                        if mcpSettings.enabled {
                            Text("\(mcpSettings.enabledTools.count)")
                                .font(.caption2)
                                .fontWeight(.medium)
                                .foregroundStyle(.white)
                                .padding(.horizontal, 6)
                                .padding(.vertical, 2)
                                .background(mcpSettings.connectionStatus.color)
                                .clipShape(Capsule())
                        }
                    }

                    Text("mcp.settings.description".localized())
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
                ...
                Button {
                    ...
                    Text("action.configure".localized())
                }
                .buttonStyle(.bordered)
            }
            .padding(.vertical, 4)
        } header: {
            Label("mcp.settings.header".localized(), systemImage: "wand.and.stars")
        } footer: {
            if mcpSettings.enabled {
                HStack(spacing: 4) {
                    Circle()
                ...
                Text("mcp.settings.disabled".localized())
                    .font(.caption)
            }
        }
